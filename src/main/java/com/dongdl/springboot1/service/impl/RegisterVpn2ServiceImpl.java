 ...
 * @description
 **/
@Service("registerVpn2ServiceImpl")
public class RegisterVpn2ServiceImpl extends RegisterServiceImpl{

    @Autowired
    IRegisterVpn2Dao iRegisterDao;
    ...
    public Map publishConfigured(Integer httpType, String saveLog, Integer belongId, String ipStr, Integer limit, IRegisterDao iRegisterDao) {
        ...
        if (list != null && list.size() > 0) {
            for (RegisterBean bean : list) {
                ...
                bean.setIpStr(ipStr);
                bean.setVpnType(Constants.INT_TWO);
                bean.valid();

                SystemLogBean systemLogBean = new SystemLogBean();
                systemLogBean.setTitle("vpn2批量注册接口");
                ...
                try {
                    ...
                    ((RegisterVpn2ServiceImpl) AopContext.currentProxy()).register(bean, this.iRegisterDao);
    ...
    public Map delListConfigured(IRegisterDao iRegisterDao) {
        ...
        if (list != null && list.size() > 0) {
            for (RegisterBean bean : list) {
                SystemLogBean systemLogBean = new SystemLogBean();
                systemLogBean.setTitle("vpn2批量删除接口");
                systemLogBean.setAction(LogActionEnum.DELETE.getName());
                systemLogBean.setParam(bean.toString());
                String message = null;
                try {
                    ((RegisterVpn2ServiceImpl) AopContext.currentProxy()).delRegisterByAtomCode(bean.getAtomCode(),
                            this.iRegisterDao);
                    successList.add(bean.getPortName());
                    systemLogBean.setOperationStatus(Constants.INT_ONE);
                } catch (Exception e) {
